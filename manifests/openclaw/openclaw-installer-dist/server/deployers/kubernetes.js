...
import { namespaceManifest, pvcManifest, configMapManifest, agentConfigMapManifest, fileTreeConfigMapManifest, fileConfigMapManifest, gcpSaSecretManifest, litellmConfigMapManifest, otelConfigMapManifest, secretManifest, serviceManifest, deploymentManifest, } from "./k8s-manifests.js";
...
async function applyNamespace(core, ns, log) {
    try {
    ...
    try {
        ...
        if (k8sApiHttpCode(e) === 403) {
            throw new Error(`Cannot create namespace "${ns}": forbidden. Create the project/namespace first (e.g. oc new-project ${ns}) and set it in the deploy form, or ask a cluster admin.`, { cause: e });
        }
        throw e;
    }
}
async function applyResource(readFn, createFn, replaceFn, name, log) {
    let exists = false;
    try {
        await readFn();
        exists = true;
...
export class KubernetesDeployer {
    async deploy(config, log) {
        ...
        await applyResource(() => core.readNamespacedPersistentVolumeClaim({ name: "openclaw-home-pvc", namespace: ns }), () => core.createNamespacedPersistentVolumeClaim({ namespace: ns, body: pvcManifest(ns) }), null, "PVC openclaw-home-pvc", log);
        // 3. ConfigMap (openclaw.json)
        const cm = configMapManifest(ns, config, gatewayToken);
        await applyResource(() => core.readNamespacedConfigMap({ name: "openclaw-config", namespace: ns }), () => core.createNamespacedConfigMap({ namespace: ns, body: cm }), () => core.replaceNamespacedConfigMap({ name: "openclaw-config", namespace: ns, body: cm }), "ConfigMap openclaw-config", log);
        // 4. ConfigMap (agent workspace files)
        const agentCm = agentConfigMapManifest(ns, config, workspaceFiles);
        await applyResource(() => core.readNamespacedConfigMap({ name: "openclaw-agent", namespace: ns }), () => core.createNamespacedConfigMap({ namespace: ns, body: agentCm }), () => core.replaceNamespacedConfigMap({ name: "openclaw-agent", namespace: ns, body: agentCm }), "ConfigMap openclaw-agent", log);
        const agentTreeCm = fileTreeConfigMapManifest(ns, "openclaw-agent-tree", agentTreeEntries);
        await applyResource(() => core.readNamespacedConfigMap({ name: "openclaw-agent-tree", namespace: ns }), () => core.createNamespacedConfigMap({ namespace: ns, body: agentTreeCm }), () => core.replaceNamespacedConfigMap({ name: "openclaw-agent-tree", namespace: ns, body: agentTreeCm }), "ConfigMap openclaw-agent-tree", log);
        const skillsCm = fileTreeConfigMapManifest(ns, "openclaw-skills", skillEntries);
        await applyResource(() => core.readNamespacedConfigMap({ name: "openclaw-skills", namespace: ns }), () => core.createNamespacedConfigMap({ namespace: ns, body: skillsCm }), () => core.replaceNamespacedConfigMap({ name: "openclaw-skills", namespace: ns, body: skillsCm }), "ConfigMap openclaw-skills", log);
        const cronCm = fileConfigMapManifest(ns, "openclaw-cron", "jobs.json", cronJobsContent);
        await applyResource(() => core.readNamespacedConfigMap({ name: "openclaw-cron", namespace: ns }), () => core.createNamespacedConfigMap({ namespace: ns, body: cronCm }), () => core.replaceNamespacedConfigMap({ name: "openclaw-cron", namespace: ns, body: cronCm }), "ConfigMap openclaw-cron", log);
        ...
        if (useProxy && litellmMasterKey) {
            ...
            await applyResource(() => core.readNamespacedConfigMap({ name: "litellm-config", namespace: ns }), () => core.createNamespacedConfigMap({ namespace: ns, body: litellmCm }), () => core.replaceNamespacedConfigMap({ name: "litellm-config", namespace: ns, body: litellmCm }), "ConfigMap litellm-config", log);
        }
        ...
        if (shouldUseOtel(config)) {
            ...
            if (operatorAvailable) {
                ...
                try {
                    ...
                    await customApi.patchNamespacedCustomObject({ ...crParams, name: "openclaw-sidecar", body: otelCr }, k8s.setHeaderOptions("Content-Type", k8s.PatchStrategy.MergePatch));
                }
                catch {
                ...
                await applyResource(() => core.readNamespacedConfigMap({ name: "otel-collector-config", namespace: ns }), () => core.createNamespacedConfigMap({ namespace: ns, body: otelCm }), () => core.replaceNamespacedConfigMap({ name: "otel-collector-config", namespace: ns, body: otelCm }), "ConfigMap otel-collector-config", log);
            }
        }
        // 5. Secret
        const secret = secretManifest(ns, config, gatewayToken, litellmMasterKey);
        await applyResource(() => core.readNamespacedSecret({ name: "openclaw-secrets", namespace: ns }), () => core.createNamespacedSecret({ namespace: ns, body: secret }), () => core.replaceNamespacedSecret({ name: "openclaw-secrets", namespace: ns, body: secret }), "Secret openclaw-secrets", log);
        ...
        if (config.gcpServiceAccountJson) {
            ...
            await applyResource(() => core.readNamespacedSecret({ name: "gcp-sa", namespace: ns }), () => core.createNamespacedSecret({ namespace: ns, body: gcpSecret }), () => core.replaceNamespacedSecret({ name: "gcp-sa", namespace: ns, body: gcpSecret }), "Secret gcp-sa", log);
        }
        // 6. Service
        const svc = serviceManifest(ns);
        await applyResource(() => core.readNamespacedService({ name: "openclaw", namespace: ns }), () => core.createNamespacedService({ namespace: ns, body: svc }), () => core.replaceNamespacedService({ name: "openclaw", namespace: ns, body: svc }), "Service openclaw", log);
        // 7. Deployment
        const dep = deploymentManifest(ns, config, otelViaOperator, skillEntries, agentTreeEntries, cronJobsContent);
        await applyResource(() => apps.readNamespacedDeployment({ name: "openclaw", namespace: ns }), () => apps.createNamespacedDeployment({ namespace: ns, body: dep }), () => apps.replaceNamespacedDeployment({ name: "openclaw", namespace: ns, body: dep }), "Deployment openclaw", log);
        ...
        try {
            ...
            writeFileSync(join(configDir, "deploy-config.json"), JSON.stringify(savedConfig, null, 2), { mode: 0o600 });
    ...
    async start(result, log) {
        ...
        await apps.patchNamespacedDeployment({ name: "openclaw", namespace: ns, body: patch }, k8s.setHeaderOptions("Content-Type", k8s.PatchStrategy.JsonPatch));
...
`.trim();
        ...
        await apps.patchNamespacedDeployment({ name: "openclaw", namespace: ns, body: patches }, k8s.setHeaderOptions("Content-Type", k8s.PatchStrategy.JsonPatch));
