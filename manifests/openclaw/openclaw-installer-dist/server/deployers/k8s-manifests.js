import { defaultImage, agentId, tryParseProjectId, buildOpenClawConfig, } from "./k8s-helpers.js";
...
export function namespaceManifest(ns) {
    return {
        apiVersion: "v1",
        kind: "Namespace",
...
export function pvcManifest(ns) {
    return {
        apiVersion: "v1",
...
export function configMapManifest(ns, config, gatewayToken) {
    return {
        apiVersion: "v1",
        kind: "ConfigMap",
        metadata: {
...
export function agentConfigMapManifest(ns, config, workspaceFiles) {
    return {
        apiVersion: "v1",
        kind: "ConfigMap",
        metadata: {
...
export function fileTreeConfigMapManifest(ns, name, entries) {
    return {
        apiVersion: "v1",
        kind: "ConfigMap",
        metadata: {
...
export function fileConfigMapManifest(ns, name, filename, content) {
    return {
        apiVersion: "v1",
        kind: "ConfigMap",
        metadata: {
...
export function gcpSaSecretManifest(ns, saJson) {
    return {
        apiVersion: "v1",
        kind: "Secret",
        metadata: {
...
export function litellmConfigMapManifest(ns, configYaml) {
    return {
        apiVersion: "v1",
        kind: "ConfigMap",
        metadata: {
...
export function otelConfigMapManifest(ns, configYaml) {
    return {
        apiVersion: "v1",
        kind: "ConfigMap",
        metadata: {
...
export function secretManifest(ns, config, gatewayToken, litellmMasterKey) {
    const data = {
    ...
    if (config.modelEndpoint)
        data.MODEL_ENDPOINT = config.modelEndpoint;
    ...
    if (projectId)
        data.GOOGLE_CLOUD_PROJECT = projectId;
    if (config.googleCloudLocation)
        data.GOOGLE_CLOUD_LOCATION = config.googleCloudLocation;
    if (litellmMasterKey)
        data.LITELLM_MASTER_KEY = litellmMasterKey;
    if (config.sandboxEnabled) {
        if (config.sandboxSshIdentity)
            data.SSH_IDENTITY = config.sandboxSshIdentity;
        if (config.sandboxSshCertificate)
            data.SSH_CERTIFICATE = config.sandboxSshCertificate;
        if (config.sandboxSshKnownHosts)
            data.SSH_KNOWN_HOSTS = config.sandboxSshKnownHosts;
    }
    return {
        apiVersion: "v1",
        kind: "Secret",
...
export function serviceManifest(ns) {
    return {
        apiVersion: "v1",
        kind: "Service",
        metadata: {
...
export function deploymentManifest(ns, config, otelViaOperator = false, skillEntries = [], agentTreeEntries = [], cronJobsContent) {
                ...
                spec: {
                    ...
                    containers: [
                        ...
                        ...(useProxy ? [{
                                ...
                                ports: [{ name: "litellm", containerPort: LITELLM_PORT, protocol: "TCP" }],
                                env: [
                                ...
                                readinessProbe: {
                                    httpGet: { path: "/health/readiness", port: LITELLM_PORT },
                        ...
                        ...(useOtelDirect ? [{
                                ...
                                ports: [
                                    { name: "otlp-grpc", containerPort: OTEL_GRPC_PORT, protocol: "TCP" },
                                    { name: "otlp-http", containerPort: OTEL_HTTP_PORT, protocol: "TCP" },
                                ],
                                volumeMounts: [
