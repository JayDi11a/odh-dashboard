...
import { discoverContainers, discoverVolumes, detectRuntime, } from "../services/container.js";
...
const router = Router();
function containerToInstance(c) {
    ...
    if (portMatch) {
        ...
        if (hostPortMatch)
            port = parseInt(hostPortMatch[1], 10);
    }
    return {
        id: c.name,
        mode: "local",
...
function decodeSavedBase64(value) {
    if (!value) {
        return undefined;
    }
    try {
...
function decodeSavedJson(value) {
...
function decodeSavedBase64UnlessPath(savedValue, savedPath) {
    if (savedPath?.trim()) {
        return undefined;
    }
    ...
    try {
        ...
        if (runtime) {
            ...
            for (const vol of volumes) {
                if (runningContainerNames.has(vol.containerName))
                    continue;
                try {
                    ...
                    instances.push({
                        ...
                        config: {
                            ...
                            inferenceProvider: savedVars.INFERENCE_PROVIDER,
                            agentSecurityMode: savedVars.AGENT_SECURITY_MODE || undefined,
                            secretsProvidersJson: decodeSavedBase64(savedVars.SECRETS_PROVIDERS_JSON_B64),
                            anthropicApiKeyRef: decodeSavedJson(savedVars.ANTHROPIC_API_KEY_REF_B64),
                            openaiApiKeyRef: decodeSavedJson(savedVars.OPENAI_API_KEY_REF_B64),
                            telegramBotTokenRef: decodeSavedJson(savedVars.TELEGRAM_BOT_TOKEN_REF_B64),
                            ...
                            sandboxBackend: savedVars.SANDBOX_BACKEND || undefined,
                            sandboxMode: savedVars.SANDBOX_MODE || undefined,
                            sandboxScope: savedVars.SANDBOX_SCOPE || undefined,
                            sandboxToolPolicyEnabled: savedVars.SANDBOX_TOOL_POLICY_ENABLED === "true" || undefined,
                            sandboxToolAllowFiles: savedVars.SANDBOX_TOOL_ALLOW_FILES === "false" ? false : undefined,
                            sandboxToolAllowSessions: savedVars.SANDBOX_TOOL_ALLOW_SESSIONS === "false" ? false : undefined,
                            sandboxToolAllowMemory: savedVars.SANDBOX_TOOL_ALLOW_MEMORY === "false" ? false : undefined,
                            sandboxToolAllowRuntime: savedVars.SANDBOX_TOOL_ALLOW_RUNTIME === "true" || undefined,
                            sandboxToolAllowBrowser: savedVars.SANDBOX_TOOL_ALLOW_BROWSER === "true" || undefined,
                            sandboxToolAllowAutomation: savedVars.SANDBOX_TOOL_ALLOW_AUTOMATION === "true" || undefined,
                            sandboxToolAllowMessaging: savedVars.SANDBOX_TOOL_ALLOW_MESSAGING === "true" || undefined,
                            sandboxWorkspaceAccess: savedVars.SANDBOX_WORKSPACE_ACCESS || undefined,
                            ...
                            sandboxSshStrictHostKeyChecking: savedVars.SANDBOX_SSH_STRICT_HOST_KEY_CHECKING === "false" ? false : undefined,
                            sandboxSshUpdateHostKeys: savedVars.SANDBOX_SSH_UPDATE_HOST_KEYS === "false" ? false : undefined,
                            sandboxSshCertificate: decodeSavedBase64UnlessPath(savedVars.SANDBOX_SSH_CERTIFICATE_B64, savedVars.SANDBOX_SSH_CERTIFICATE_PATH),
                            sandboxSshKnownHosts: decodeSavedBase64UnlessPath(savedVars.SANDBOX_SSH_KNOWN_HOSTS_B64, savedVars.SANDBOX_SSH_KNOWN_HOSTS_PATH),
                        },
                        startedAt: "",
...
router.post("/:id/redeploy", async (req, res) => {
    ...
    if (!("redeploy" in deployer) || typeof deployer.redeploy !== "function") {
        res.status(400).json({ error: "Use Stop/Start for this deployer — redeploy is not supported" });
        return;
    }
    const log = createLogCallback(instance.id);
    try {
        await (deployer.redeploy(instance, log));
        sendStatus(instance.id, "running");
...
router.get("/:id/command", async (req, res) => {
    ...
    try {
        ...
        for (const e of envList) {
            if (e.startsWith("PATH=") || e.startsWith("HOSTNAME=") || e.startsWith("container="))
                continue;
 ...
 */
async function readSavedConfig(containerName) {
    const vars = {};
    try {
 ...
 */
async function savedDeployMode(namespace) {
    try {
        ...
        if (vol) {
            ...
            return {
                ...
                config: {
                    ...
                    vertexProvider: savedVars.VERTEX_PROVIDER || undefined,
