 ...
 * resources.
 */
export class OpenShiftDeployer {
    k8s = new KubernetesDeployer();
    async deploy(config, log) {
            ...
            if (status === 403) {
                ...
                try {
                    ...
                    if (k8sApiHttpCode(createErr) === 403) {
                        throw new Error(`Cannot create namespace "${ns}": forbidden. Create the project first (for example: oc new-project ${ns}) and set it in the deploy form, or ask a cluster admin.`, { cause: createErr });
                    }
                    throw createErr;
                }
            }
        ...
        await applyResource(() => core.readNamespacedServiceAccount({ name: "openclaw-oauth-proxy", namespace: ns }), () => core.createNamespacedServiceAccount({ namespace: ns, body: sa }), () => core.replaceNamespacedServiceAccount({ name: "openclaw-oauth-proxy", namespace: ns, body: sa }), "ServiceAccount openclaw-oauth-proxy", log);
        // OAuth config secret (client-secret + cookie_secret)
        const oauthSecret = await oauthConfigSecret(ns);
        await applyResource(() => core.readNamespacedSecret({ name: "openclaw-oauth-config", namespace: ns }), () => core.createNamespacedSecret({ namespace: ns, body: oauthSecret }), null, // Don't replace — keep existing secrets on re-deploy
        "Secret openclaw-oauth-config", log);
        ...
        const svc = {
            ...
            spec: {
                ...
                ports: [
                    ...
                    { name: "oauth-ui", port: 8443, targetPort: 8443, protocol: "TCP" },
                ],
            },
        };
        ...
        await apps.patchNamespacedDeployment({ name: "openclaw", namespace: ns, body: deployPatch }, k8s.setHeaderOptions("Content-Type", k8s.PatchStrategy.JsonPatch));
