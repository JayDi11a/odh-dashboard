...
import { shouldUseOtel, generateOtelConfig, otelAgentEnv, OTEL_COLLECTOR_IMAGE, JAEGER_IMAGE, JAEGER_UI_PORT, } from "./otel.js";
const execFileAsync = promisify(execFile);
const OTEL_STATE_DIR = "/otel-state";
const OTEL_CONFIG_PATH = `${OTEL_STATE_DIR}/otel/config.yaml`;
export function otelContainerName(config) {
 ...
 */
export async function startOtelSidecar(config, runtime, volumeName, podNameOrNull, litellmContainerOrNull, port, image, log, runCommand, removeContainer) {
 ...
 */
export async function stopOtelSidecar(config, runtime, log, runCommand) {
    const otelName = otelContainerName(config);
    try {
...
export function jaegerContainerName(config) {
    const prefix = config.prefix || "openclaw";
 ...
 */
export async function startJaegerSidecar(config, runtime, podNameStr, log, runCommand, removeContainer) {
