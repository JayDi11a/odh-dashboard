...
import { detectRuntime, removeContainer, removeVolume, OPENCLAW_LABELS, } from "../services/container.js";
import { shouldUseLitellmProxy, litellmModelName, generateLitellmMasterKey, generateLitellmConfig, LITELLM_IMAGE, LITELLM_PORT, } from "./litellm.js";
...
export function shouldAlwaysPull(image) {
...
function tryParseProjectId(saJson) {
    try {
...
function resolveOptionalTextFile(filePath) {
...
function prepareLocalSandboxSshConfig(config) {
...
function shouldAutoEnvRef(explicitRef, value) {
...
function buildOpenClawConfig(config, gatewayToken) {
 ...
 */
function buildDefaultAgentsMd(config) {
    const agentId = `${config.prefix || "openclaw"}_${config.agentName}`;
 ...
 */
function buildAgentJson(config) {
    const agentId = `${config.prefix || "openclaw"}_${config.agentName}`;
    const displayName = config.agentDisplayName || config.agentName;
...
function litellmContainerName(config) {
    return `${containerName(config)}-litellm`;
}
function podName(config) {
    return `${containerName(config)}-pod`;
}
function volumeName(config) {
    const prefix = config.prefix || "openclaw";
...
function runCommand(cmd, args, log) {
    return new Promise((resolve, reject) => {
        ...
        const redacted = args.map((a, i) => args[i - 1] === "-e" &&
            /^(ANTHROPIC_API_KEY|OPENAI_API_KEY|TELEGRAM_BOT_TOKEN|SSH_IDENTITY|SSH_CERTIFICATE|SSH_KNOWN_HOSTS)=/.test(a)
            ? a.replace(/=.*/, "=***")
            : a);
...
function defaultAgentSourceDir(isContainerized) {
    if (isContainerized) {
        return null;
    }
 ...
 */
function buildRunArgs(config, runtime, name, port, litellmMasterKey, otelEnvVars) {
    ...
    runArgs.push("--label", OPENCLAW_LABELS.managed, "--label", OPENCLAW_LABELS.prefix(effectiveConfig.prefix || "openclaw"), "--label", OPENCLAW_LABELS.agent(effectiveConfig.agentName));
    const env = {
        HOME: "/home/node",
        NODE_ENV: "production",
    };
...
export class LocalDeployer {
    async deploy(config, log) {
        ...
        if (!runtime) {
            throw new Error("No container runtime found. Install podman or docker first.");
        }
...
*(populated through experience)*`;
        ...
        if (config.otelJaeger) {
            await startJaegerSidecar(config, runtime, podName(config), log, runCommand, (rt, nm) => removeContainer(rt, nm));
        }
        // Start OTEL collector sidecar if enabled
        const otelEnv = await startOtelSidecar(config, runtime, vol, (useProxy || useOtelSidecars) ? podName(config) : null, useProxy ? litellmContainerName(config) : null, port, image, log, runCommand, (rt, nm) => removeContainer(rt, nm));
        ...
        if (hasSidecars) {
            ...
            if (otelEnv)
                log(`OTEL container:    ${otelContainerName(config)}`);
            if (config.otelJaeger)
                log(`Jaeger container:  ${jaegerContainerName(config)}`);
            log("");
            if (config.otelJaeger)
            ...
            if (otelEnv)
                log(`  ${runtime} logs ${otelContainerName(config)}  # OTEL collector logs`);
            if (config.otelJaeger)
                log(`  ${runtime} logs ${jaegerContainerName(config)}  # Jaeger logs`);
        }
        else {
            log(`Container: ${name}`);
            log("");
    ...
    async start(result, log) {
        ...
        if (effectiveConfig.otelJaeger) {
            await startJaegerSidecar(effectiveConfig, runtime, podName(effectiveConfig), log, runCommand, (rt, nm) => removeContainer(rt, nm));
        }
        // Restart OTEL sidecar if enabled
        const otelEnv = await startOtelSidecar(effectiveConfig, runtime, vol, (useProxy || useOtelSidecars) ? podName(effectiveConfig) : null, useProxy ? litellmContainerName(effectiveConfig) : null, port, image, log, runCommand, (rt, nm) => removeContainer(rt, nm));
        log(`Starting OpenClaw container: ${name}`);
     ...
     */
    async saveInstanceInfo(runtime, name, config, log, precomputedToken) {
        ...
        // Save .env
        try {
            const encodeEnvValue = (value) => Buffer.from(value, "utf8").toString("base64");
            const lines = [
            ...
            if (config.sandboxEnabled) {
                ...
                lines.push(`SANDBOX_SSH_STRICT_HOST_KEY_CHECKING=${config.sandboxSshStrictHostKeyChecking !== false}`);
