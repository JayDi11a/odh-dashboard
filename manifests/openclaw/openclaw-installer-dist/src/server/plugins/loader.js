...
export async function getDisabledModes() {
    const config = await readPluginConfig();
    if (Array.isArray(config.disabled)) {
        return config.disabled.filter((m) => typeof m === "string");
    }
    return [];
}
let configLock = Promise.resolve();
export async function setModeDisabled(mode, disabled) {
    ...
    try {
        const config = await readPluginConfig();
        const current = new Set(Array.isArray(config.disabled)
            ? config.disabled.filter((m) => typeof m === "string")
            : []);
        if (disabled) {
            current.add(mode);
        }
...
async function discoverProviderPlugins(registry) {
...
async function loadConfigPlugins() {
    const config = await readPluginConfig();
    if (Array.isArray(config.plugins)) {
        return config.plugins.filter((p) => typeof p === "string");
    }
    return [];
}
async function loadPlugin(registry, moduleId, source) {
...
export async function loadPlugins(registry) {
    ...
    for (const pluginId of allPlugins) {
        const source = npmSet.has(pluginId) ? "npm" : "config";
