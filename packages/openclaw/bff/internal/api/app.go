...
import (
	...
	k8s "github.com/opendatahub-io/odh-dashboard/packages/openclaw/bff/internal/integrations/kubernetes"
	k8mocks "github.com/opendatahub-io/odh-dashboard/packages/openclaw/bff/internal/integrations/kubernetes/k8mocks"
	"k8s.io/client-go/kubernetes"
	"sigs.k8s.io/controller-runtime/pkg/envtest"

	helper "github.com/opendatahub-io/odh-dashboard/packages/openclaw/bff/internal/helpers"

	"github.com/opendatahub-io/odh-dashboard/packages/openclaw/bff/internal/config"
	"github.com/opendatahub-io/odh-dashboard/packages/openclaw/bff/internal/integrations/deployer"
	"github.com/opendatahub-io/odh-dashboard/packages/openclaw/bff/internal/repositories"
...
func NewApp(cfg config.EnvConfig, logger *slog.Logger) (*App, error) {
	...
	// Initialize deployer client
	deployerServiceURL := os.Getenv("DEPLOYER_SERVICE_URL")
	if deployerServiceURL == "" {
		deployerServiceURL = "http://localhost:3100"
	}
	deployerClient := deployer.NewClient(deployerServiceURL)
...
func (app *App) Routes() http.Handler {
	...
	// OpenClaw instance management endpoints
	apiRouter.POST("/api/instances", app.DeployInstanceHandler)
	apiRouter.GET("/api/instances", app.ListInstancesHandler)
	apiRouter.GET("/api/instances/:name", app.GetInstanceHandler)
	apiRouter.DELETE("/api/instances/:name", app.DeleteInstanceHandler)
	...
	// Create a mux for public API endpoints (no auth required)
	publicMux := http.NewServeMux()
	publicRouter := httprouter.New()
	publicRouter.GET("/api/v1/detector", app.DetectorHandler)
	publicMux.Handle("/api/v1/detector", app.RecoverPanic(app.EnableTelemetry(app.EnableCORS(publicRouter))))
	...
	combinedMux.Handle("/api/v1/detector", publicMux)
